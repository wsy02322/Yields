...
def estimate_block_for_timestamp(
    ...
    *,
    tip_number: int,
    tip_ts: int,
    avg_block_time: float = 12.08,
) -> int:
    """Fast block estimate (no extra RPC). Prefer slightly earlier block."""
    if target_ts >= tip_ts:
        return tip_number
    delta = tip_ts - target_ts
    est = tip_number - int(delta / avg_block_time)
    return max(0, min(est, tip_number))


def retry_call(fn, retries: int = 6, base_sleep: float = 0.75):
    last = None
